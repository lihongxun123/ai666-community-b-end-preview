...
本目录是 AI666 / 多元拾光社区 B端 V0.1 的 Ant Design 风格多页静态 HTML 原型。当前原型用于验证信息架构、导航分配、24 个左侧导航入口、7 个详情/配置工作页、25 个正式 C端页面承接，以及内容、模型、活动、赛事、展示投放、用户、积分商城、权限与日志的处理闭环。
...
- 5 个一级业务组：内容管理、AIGC管理、运营管理、用户管理、系统管理。
- 24 个左侧导航入口和 7 个详情/配置工作页均为独立 HTML 文件，导航为真实页面跳转，并带页面契约。
- 覆盖 `scripts/community-public-preview-config.mjs` 中 25 个正式 C端页面，并归类 2 个独立预览入口。
