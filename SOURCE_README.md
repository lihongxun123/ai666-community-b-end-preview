...
本目录是 AI666 / 多元拾光社区 B端 V0.1 的 Ant Design 风格多页静态 HTML 原型。当前原型用于验证信息架构、导航分配、32 个左侧导航入口、8 个详情/配置工作页、29 个正式 C端页面承接，以及内容、模型、活动、赛事、展示投放、用户、积分商城、统计分析、权限与日志的处理闭环。

## 来源

```text
docs/product/AI666_B端管理后台原型功能设计规范.md
docs/product/AI666_C端B端页面联动规则精简版.md
docs/versions/v1.0/business-logic/cross-page/AI666_统计分析第一期PRD_V0.1.md
docs/versions/v1.0/status/modules/community-b-end-admin.md
```
...
- 6 个一级业务组：内容管理、AIGC管理、运营管理、用户管理、数据分析、系统管理。
- 32 个左侧导航入口和 8 个详情/配置工作页均为独立 HTML 文件，当前共生成 40 页；导航为真实页面跳转，并带页面契约。
- 覆盖 `scripts/community-public-preview-config.mjs` 中 29 个正式 C端页面；独立预览入口按当前 C端预览配置另行管理。
...
- 赛事管理与赛事工作台独立于常规活动模型，承接报名、阶段动作矩阵、投稿及版本、评分与下一阶段条件、作品治理、榜单和 9 个 MakeNow P0 对接状态；活动中心只保留赛事入口。
...
- 数据分析第一期八页已全部进入左侧导航和生成页，包括数据总览、增长分析、用户分析、内容生态、创作分析、活动与激励、运营质量、指标中心；各页均明确标注演示数据、数据截止时间、来源边界和只读回查入口，不做复杂 BI，也不把聚合对照表述为个体归因。
